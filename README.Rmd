...
In `predShortVersion$pred` is a summary of the predictions (e.g., the predicted mean and true scores, predicted probabilities and true decisions). In `predShortVersion$distFun` we have the approximated joint distribution of the scores and corresponding decisions. For two scores and decision probabilistic predictions based on `predShortVersion$distFun` can be visualized with the function `plotScoresItemSelection`. The results are: 
